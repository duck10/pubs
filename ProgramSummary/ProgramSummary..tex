...
\begin{document}                  % DO NOT DELETE THIS LINE
		...
		\begin{tabular}{l p{.2\textwidth} l l l}
			...
			\begin{tabular}{l l l l}
				...
				\parbox[t]{0.6\textwidth}{\textbf{g} 
					(or \textbf{v} or \textbf{g6}) for \GVI{} vectors\\
					\textbf{d} (or \textbf{d7}) for \DVII{} vectors\\
					\textbf{s} (or s6) for \SVI{}, Delone/Selling scalars\\
			...
			\textbf{Crystal Lattice Input:} 
			``\textbf{A}'', ``\textbf{B}'', ``\textbf{C}'', ``\textbf{P}'', ``\textbf{R}'', ``\textbf{F}'', ``\textbf``{I}''	\\
	...
	\begin{table}
		...
		\begin{tabular}{l l l p{.2\textwidth} p{.5\textwidth} l}
			\toprule
	...
	\end{table}
		...
		\begin{tabular}{l l l p{.2\textwidth} p{.5\textwidth} l}
			...
			Name&	in&	out&	param& Output	\\
			\midrule
			CmdCmplx&	NA&	y&&Currently, this just outputs some 
			programmed examples.		\\[.9pt]
			CmdDelone&	y&	\SVI{}&&Converts input to\SVI{}		\\[.9pt]
			CmdGen&	NA&	\GVI{}&	ngen& - number of examples to generate (optional)\\
			{}&&&type - &lattice type to generate, many options (optional) 
			c,t,h,o,r,h,m,a, 
			...
			For usage examples, see Table \ref{CMDGENexamples}\\[.9pt]
					CmdLM&	y&	cy&&		Lattice Matching. 
			The first input cell is used as the reference. 
			Succeeding cells are matched 
			as well as possible to the reference cell.\\[.9pt]
			CmdNiggli&	y&	\GVI{}&&		Produces the Niggli reduced cell for each input\\[.9pt]
			CmdPath&	?&	? && Clearly not working the way it should		\\[.9pt]
			CmdPerturb&	y&	y&	n-number of perturbations, parts per thousand to perturb&
			...
			CmdS6Refl&	y&	\SVI{}&	&	For each input, 
			all 24 \SVI{} reflections are produced.\\[.9pt]
			CmdSella&	y&	y		\\[.9pt]
			CmdSort&	y&	y&	"C3" or "seq'&	\\[.9pt]
			CmdVolume&	y&	mod&&		Outputs the unit cell and the volume of each input cell.\\[.9pt]
			MultiMetricDists&	y& mod&"Y/y"(optional)&	Parameter Y/y for maxima style output		\\[.9pt]
			PlotC3&	y&	mod& 
			1,2,or 3 for which \CIII{} coordinate to plot. 
			"seq" or "C3"& Plot one of the 3 \CIII{} coordinates, 
			sorted by minimizing point to point distance 
