...
\section*{Definitions: Reduction Behavior Categories}

Each boundary polytope in the Niggli-reduced region of $G^6$ is classified by its reduction behavior. These categories reflect the stability, uniqueness, and reproducibility of the reduced cell under Krivy--Gruber reduction and guide classifier overrides.

\subsection*{Hard}

A boundary is classified as \textbf{Hard} if it exhibits intrinsic instability under reduction. This includes:

\begin{itemize}
	\item \textbf{Non-uniqueness:} Multiple unimodular transformations yield distinct but valid reduced cells.
	\item \textbf{Perturbation sensitivity:} Infinitesimal changes in the metric tensor cause discontinuous shifts in the reduced cell.
	\item \textbf{Oscillatory behavior:} Reduction may cycle or fail to converge near the boundary.
	\item \textbf{Classifier override required:} Reduction alone cannot guarantee correct Bravais assignment.
\end{itemize}

Hard boundaries typically involve compound degeneracies, saturated inequalities, or cubic manifolds.

\subsection*{Conditionally Hard}

A boundary is classified as \textbf{Conditionally Hard} if it is stable in isolation but may become unstable when intersected with other boundaries. These cases include:

\begin{itemize}
	\item \textbf{Single saturations:} Exact equality in one Niggli condition (e.g., $|g_4| = g_2$)
	\item \textbf{Edge equalities with orthogonality:} Stable unless combined with near-degeneracy
	\item \textbf{Ambiguous sorting:} Near-equalities that may flip axis order under perturbation
\end{itemize}

Classifier pipelines must monitor adjacency and apply parametric triggers when multiple such conditions co-occur.

\subsection*{Soft}

A boundary is classified as \textbf{Soft} if it is stable, reproducible, and reduction-consistent. These boundaries:

\begin{itemize}
	\item Yield a unique reduced cell under Krivy--Gruber reduction
	\item Are insensitive to small perturbations
	\item Do not require classifier intervention
\end{itemize}

Soft boundaries include orthogonality conditions (e.g., $g_4 = 0$) and simple edge equalities (e.g., $g_1 = g_2$) that do not intersect with other instability zones.

\subsection*{Usage in Classifier Pipeline}

These categories are encoded in the \textbf{Reduction Behavior} column of the boundary table and inform:

\begin{itemize}
	\item Instability tagging (S, C, U)
	\item Symbolic trigger logic
	\item Override and fallback mechanisms
\end{itemize}

They ensure that classification remains robust across the full geometry of the Niggli cone in $G^6$.
...
\begin{center}
	\begin{tabular}{|c|p{4.5cm}|p{6cm}|}
		\hline
		\textbf{Zone} & \textbf{Typical Conditions} & \textbf{Classifier Implication} \\
		\hline
		Soft & $g_1 = g_2$, $g_4 = 0$ & Stable reduction; no override needed \\
		\hline
		Conditionally Hard & $|g_4| = g_2$, $g_1 \approx g_2$, $g_1 = g_2$, $g_4 = 0$ & Stable unless intersected; monitor adjacency and apply parametric triggers \\
		\hline
		Hard & $g_1 = g_2 = g_3$, $g_4 = g_5 = g_6 = 0$, $g_1 = g_5 = g_6$ & Intrinsically unstable; classifier override required \\
		\hline
	\end{tabular}
\end{center}


% This cell is invalid, apparently Polar::rand generates bad cells occasionally
%\subsection{33277}testCase(np, "case 33277", G6(6.486, 46.833, 30.720, -37.984, 27.277, -24.644), true);
%\subsubsection{33277}
%gross failure in both
%Output:    -1.272     0.088     0.357    -0.924    -0.805    -1.515
%Converged: YES
%Is Niggli reduced: NO
%Standard Niggli:        0.088     0.331    -0.479    -1.572     0.748     1.691


\subsection{10130}testCase(np, "case 10130", G6(78.145, 6.409, 1.485, -6.169, -4.536, 8.971), true);
\subsubsection[10130]
testCase(np, "acute", G6(0.825,     1.000,    65.850,     0.611,     0.051,     0.825), true);
converged at 100, but wandered a lot
  Iteration 99:     0.011     0.295    51.361     0.282     0.015     0.009
Output:     0.011     0.295    51.357    -0.273    -0.007    -0.009
Converged: YES
Is Niggli reduced: YES
Standard Niggli:        0.011     0.295    51.357    -0.273    -0.007    -0.009
Max difference: 6.39488e-14
? MATCH


Iteration 100:     0.011     0.295    51.357    -0.273    -0.007    -0.009
Checking all boundaries:
IsNiggli true
g     0.011     0.295    51.357    -0.273    -0.007    -0.009

Checking boundaries:
Checking boundaries: 8 (0.022)
Checking boundaries: D' (0) 7' (0) A' (0) 8' (0.0179722) B' (0.00943398) E' (0.00714143)     CONVERGED
...
\subsection{acute}
   testCase(np, "acute", G6(0.825,     1.000,    65.850,     0.611,     0.051,     0.825), true);
\subsection{acute}
Output:     0.825     1.000    65.850    -0.560    -0.051    -0.825
Converged: YES
Is Niggli reduced: YES
Standard Niggli:        0.825     1.000    65.850     0.611     0.051     0.825
Max difference: 1.65
? 
...
\subsection{testCase(np, "obtuse", G6(78.145, 6.409, 1.485, -6.169, -4.536, 8.971), true);}
\subsection[obtuse]
======================================================================
Test: obtuse
Input:     78.145     6.409     1.485     0.000     0.000     0.000
...
Initial Phase 1:
Phase1: Pattern 111, applying M2, M1
Phase1: Pattern 010, applying M2
Phase1 complete after 3 iterations
Phase1 complete after 2 iterations

Iteration 0:     1.485     6.409    78.145     0.000     0.000     0.000
Checking all boundaries:
IsNiggli true
g     1.485     6.409    78.145     0.000     0.000     0.000
3 (6.409) 4 (0) 5 (0)
Checking boundaries: 6/7 (6.409) 9/A (1.485) C/D (1.485)
Checking boundaries: 8 (6.409)
Checking boundaries: D' (0) 7' (0) A' (0) 8' (0) B' (0) E' (0)     CONVERGED

Output:     1.485     6.409    78.145     0.000     0.000     0.000
Converged: YES
Is Niggli reduced: YES
Standard Niggli:        1.485     6.409    78.145     0.000     0.000     0.000
Max difference: 0
? MATCH
