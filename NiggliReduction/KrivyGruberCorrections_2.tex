...
\usepackage{amsmath,amssymb,algorithm,algorithmic,booktabs,siunitx}
...
\begin{document}
	...
	This corresponds to a primitive cell with volume $V_{\text{in}} = 50.073$~\si{\angstrom}$^3$.
	...
	Therefore $V_{\text{out}} = 20.48$~\si{\angstrom}$^3$, which is only 40.9\% of the input volume. This non-unimodular transformation generates a \textbf{different lattice}.
	...
	Therefore $V_{\text{corrected}} = 50.10$~\si{\angstrom}$^3$ $\approx V_{\text{in}}$, confirming volume preservation.
	...
	\begin{table}[h]
		...
		\begin{tabular}{lcccccc}
			...
			\midrule
			Volume (input) & \multicolumn{6}{c}{50.073~\si{\angstrom}$^3$} \\
			Volume (published) & \multicolumn{6}{c}{20.469~\si{\angstrom}$^3$ \textbf{(40.9\% error)}} \\
			Volume (corrected) & \multicolumn{6}{c}{50.097~\si{\angstrom}$^3$ (0.05\% error)} \\
			\midrule
	...
	\begin{enumerate}
		\item \textbf{Abstract presentation:} The algorithm was presented in terms of scalar invariants $(A,B,C,\xi,\eta,\zeta)$ rather than explicit matrix transformations or basis vectors, making geometric interpretation difficult.
		...
		\item \textbf{Typographical assumption:} The error in Step 1 involves a single character ($\xi$ vs.\ $\zeta$). Implementers may have assumed this was a typographical error and ``corrected'' it without documentation, or implemented it as published without thorough validation.
		...
		\begin{itemize}
			...
			\item Verifying that the transformation is unimodular (determinant $\pm 1$)
	...
	\begin{enumerate}
		\item \textbf{Error 1 (Critical):} Incorrect axis swap in Step 1 causes non-unimodular transformations and volume non-preservation. The published text should read ``change $(A,\xi)\leftrightarrow(B,\eta)$'' not ``change $(A,\zeta)\leftrightarrow(B,\eta)$''.
	...
	\begin{itemize}
		...
		\item Executes 5.5× faster than Andrews-Bernstein
	...
	\begin{thebibliography}{99}
		...
		G. Eisenstein,
		\textit{Über die Reduction der quadratischen Formen},
